    ...
    <TeamShell className="map-page">
      ...
      <section className="map-filter">
        <label>
          ...
          <button type="button" className={lost ? "toggle on lost" : "toggle lost"} onClick={() => setLost(!lost)} />
        </label>
        <label>
          <MapPin size={16} /> Bulunan Hayvanlar
          <button type="button" className={found ? "toggle on found" : "toggle found"} onClick={() => setFound(!found)} />
        </label>
      </section>
      ...
      <section className="map-art">
        ...
        {visibleListings.map((item, index) => (
          <div key={item.id} className={`map-marker ${index % 2 === 0 ? "marker-one" : "marker-two"}`}>
            <MapPin size={18} />
