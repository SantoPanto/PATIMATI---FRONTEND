...
import type { VetClinicReviewResponse } from "../services/types";
import {
  deleteMyClinicReview,
  listClinicReviews,
  upsertMyClinicReview,
} from "../services/vetReviews";
...
/**
 * Klinik puan/yorum listesi + gönderim formu (plan §12, §14).
 * `PetNotesSection`'ın "liste + satır-içi form" ruhu ama ayrı bileşen --
 * yıldız girişi ve "yazar başına tek yorum, upsert" semantiği farklı.
 *
 * Klinik sahibinin kendi kliniğine yorum yapamaması SUNUCU tarafında
 * uygulanır (`BusinessException`) -- bu bileşen yalnızca `clinicId` alır,
 * görüntüleyenin klinik sahibi olup olmadığını bilemez; sahip yine de
 * denerse gönderim hatası (`errorMessage`) olarak görünür.
 */
export default function VetClinicReviewsSection({ clinicId }: { clinicId: number }) {
  const { isAuthenticated } = useAuth();

  const [reviews, setReviews] = useState<VetClinicReviewResponse[]>([]);
  ...
  const loadReviews = () => {
    setIsLoading(true);
    return listClinicReviews(clinicId)
  ...
  const startEdit = (review: VetClinicReviewResponse) => {
  ...
  const handleUpdate = async (reviewId: number) => {
    ...
    try {
      await upsertMyClinicReview(clinicId, {
        rating: editRating,
  ...
  const handleDelete = async (reviewId: number) => {
    ...
    setErrorMessage(null);
    try {
      await deleteMyClinicReview(clinicId);
      await loadReviews();
    } catch {
  ...
  return (
    <div className="space-y-4">
      ...
      {isAuthenticated && (
        <div
          data-testid="vet-clinic-review-form"
