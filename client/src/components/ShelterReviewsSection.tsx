...
import type { ShelterReviewResponse } from "../services/types";
import {
  deleteMyShelterReview,
  listShelterReviews,
  upsertMyShelterReview,
} from "../services/shelterReviews";
...
/**
 * Barınak puan/yorum listesi + gönderim formu -- `VetClinicReviewsSection.tsx`nin
 * satır satır aynısı, `{clinicId}` yerine `{shelterId}` prop'u (plan §13).
 * Barınak SEVİYESİNDE puanlama -- Petshop'taki ürün bazlı desenden farklı.
 *
 * Barınak sahibinin kendi barınağına yorum yapamaması SUNUCU tarafında
 * uygulanır (`BusinessException`) -- bu bileşen yalnızca `shelterId` alır,
 * görüntüleyenin barınak sahibi olup olmadığını bilemez; sahip yine de
 * denerse gönderim hatası (`errorMessage`) olarak görünür.
 */
export default function ShelterReviewsSection({ shelterId }: { shelterId: number }) {
  const { isAuthenticated } = useAuth();

  const [reviews, setReviews] = useState<ShelterReviewResponse[]>([]);
  ...
  const loadReviews = () => {
    setIsLoading(true);
    return listShelterReviews(shelterId)
  ...
  const startEdit = (review: ShelterReviewResponse) => {
  ...
  const handleUpdate = async (reviewId: number) => {
    ...
    try {
      await upsertMyShelterReview(shelterId, {
        rating: editRating,
  ...
  const handleDelete = async (reviewId: number) => {
    ...
    setErrorMessage(null);
    try {
      await deleteMyShelterReview(shelterId);
      await loadReviews();
    } catch {
  ...
  return (
    <div className="space-y-4">
      ...
      {isAuthenticated && (
        <div
          data-testid="shelter-review-form"
