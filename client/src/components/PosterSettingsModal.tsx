...
export default function PosterSettingsModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update state immediately without page reload (Optimistic UI)
      onSuccess(updatedAd || { ...ad, ...payload });
  ...
  return (
    ...
    >
      <section
        ...
        aria-modal="true"
        className="w-full max-w-lg overflow-hidden rounded-[28px] border border-white/70 bg-white shadow-2xl shadow-slate-950/25"
        role="dialog"
      ...
      >
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="p-6 space-y-6">
          ...
          {/* Bağımlı Seçenekler (Sadece ana toggle açıkken aktif/görünür) */}
          {isPosterAllowed && (
