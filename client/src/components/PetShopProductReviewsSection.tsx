...
import type { PetShopProductReviewResponse } from "../services/types";
import {
  deleteMyProductReview,
  listProductReviews,
  upsertMyProductReview,
} from "../services/petshopProductReviews";
...
/**
 * Ürün puan/yorum listesi + gönderim formu -- `VetClinicReviewsSection.tsx`nin
 * neredeyse birebir kopyası, `{clinicId}` yerine `{productId}` prop'u
 * (plan §11).
 *
 * Dükkan sahibinin kendi ürününe yorum yapamaması SUNUCU tarafında
 * uygulanır (`BusinessException`) -- bu bileşen yalnızca `productId` alır,
 * görüntüleyenin dükkan sahibi olup olmadığını bilemez; sahip yine de
 * denerse gönderim hatası (`errorMessage`) olarak görünür.
 */
export default function PetShopProductReviewsSection({ productId }: { productId: number }) {
  const { isAuthenticated } = useAuth();

  const [reviews, setReviews] = useState<PetShopProductReviewResponse[]>([]);
  ...
  const startEdit = (review: PetShopProductReviewResponse) => {
  ...
  const handleUpdate = async (reviewId: number) => {
    ...
    try {
      await upsertMyProductReview(productId, {
        rating: editRating,
  ...
  return (
    <div className="space-y-4">
      ...
      {isAuthenticated && (
        <div
          data-testid="petshop-product-review-form"
